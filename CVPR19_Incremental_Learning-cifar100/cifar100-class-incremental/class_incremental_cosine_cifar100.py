...
parser.add_argument('--nb_cl_fg', default=50, type=int, \
    help='the number of classes in first group(第一组中的类数)')
parser.add_argument('--nb_cl', default=10, type=int, \
    help='Classes per group(每组类)')
parser.add_argument('--nb_protos', default=20, type=int, \
    help='Number of prototypes per class at the end(结束时每个类的原型数)')
parser.add_argument('--nb_runs', default=1, type=int, \
    help='Number of runs (random ordering of classes at each run):运行次数（每次运行时类的随机顺序）')
