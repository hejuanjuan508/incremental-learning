...
base_lr                = 0.1            # Initial learning rate 初始学习速率
lr_strat               = [80, 120]      # Epochs where learning rate gets decreased 学习率下降的时期
lr_factor              = 0.1            # Learning rate decrease factor  学习率下降因素
custom_weight_decay    = 5e-4           # Weight Decay 权衰减
custom_momentum        = 0.9            # Momentum  动量
...
for iteration_total in range(args.nb_runs):
    ...
    # Initialization of the variables for this run初始化此运行的变量
    X_valid_cumuls    = []
    X_protoset_cumuls = []
    X_train_cumuls    = []
    Y_valid_cumuls    = []
    ...
    #下面包含不同类的所有训练样本，因为我们想将我们的方法与存储所有训练样本的理论案例进行比较
    ...
    for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
        ...
        # Prepare the training data for the current batch of classes准备当前批次课程的培训数据
        ...
        # Add the stored exemplars to the training data将存储的示例添加到培训数据中
        if iteration == start_iter:
            X_valid_ori = X_valid
            Y_valid_ori = Y_valid
        else:
        ...
        # Launch the training loop 启动训练循环
        ...
        # Herding
        print('Updating exemplar set...') #更新示例集
        for iter_dico in range(last_iter*args.nb_cl, (iteration+1)*args.nb_cl):
            # Possible exemplars in the feature space and projected on the L2 sphere特征空间中投影在L2球面上的可能示例
            evalset.data = prototypes[iter_dico].astype('uint8')
            evalset.targets = np.zeros(evalset.data.shape[0]) #zero labels零标签
            ...
            # Herding procedure : ranking of the potential exemplars羊群过程：潜在样本的排序
            mu  = np.mean(D,axis=1)
        ...
        # Prepare the protoset准备原型集
        X_protoset_cumuls = []
        Y_protoset_cumuls = []

        # Class means for iCaRL and NCM + Storing the selected exemplars in the protoset用于iCaRL和NCM+的类方法，将选定的示例存储在原型集中
        ...
        for iteration2 in range(iteration+1):
            for iter_dico in range(args.nb_cl):
                ...
                # Collect data in the feature space for each class在要素空间中为每个类收集数据
        ...
        print('Computing accuracy on the original batch of classes...')#原始批类的计算精度
        ...
        map_Y_valid_cumul = np.array([order_list.index(i) for i in Y_valid_cumul])
        print('Computing cumulative accuracy...')#计算累积精度
        ...
        print('Computing confusion matrix...')#计算混淆矩阵
