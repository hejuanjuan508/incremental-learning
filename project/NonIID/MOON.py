...
def MOON_ClientUpdate(args, lr, client, netS_local, netC_local,global_model,previous_netS, labeled_train_loader, fed_round):
    ...
    if args.use_cuda:
        ...
        previous_netS = copy.deepcopy(previous_netS).to('cuda')
    netS_local.train()
    ...
    for epoch in range(1, args.epochs + 1):
        for batch_idx, data in tqdm(enumerate(labeled_train_loader, 1)):
            ...
            #模型对比损失函数
            rep_global = global_model(image_s1)
            rep_global = F.sigmoid(rep_global)

            rep_old = previous_netS(image_s1)
            rep_old = F.sigmoid(rep_old)

            loss_con = - torch.log(torch.exp(F.cosine_similarity(output_s1, rep_global) / args.temperature) / (
                    torch.exp(F.cosine_similarity(output_s1, rep_global) / args.temperature) + torch.exp(
                F.cosine_similarity(output_s1, rep_old) / args.temperature)))

            loss_G_joint = loss_G1 + 0.1*loss_dice+args.mu * torch.mean(loss_con)
