...
from NonIID.Local import local_train_dataset
from fed_main1 import fed_main1


#选择使用第几块GPU
from tool.FedDataset import FedDataset
from tool.utils import crete_client2, covid_ked, plot1
...
if __name__ == '__main__':
    ...
    path1 = [test_path1, test_path2, test_path3, test_path4]
    #合并所有的dataset
    lungs, infections = crete_client2(path, args.size)
    lungs1, infections1 = crete_client2(path1, args.size)
    print(lungs.shape, infections.shape)
    print(lungs1.shape, infections1.shape)
    seq = iaa.Sequential([
    ...
    ensemble_train = FedDataset(seq,lungs, infections, args.size, flag=0,weak=True)
    ensemble_test = FedDataset(seq, lungs1, infections1, args.size, flag=0, weak=True)
    print("-----------------plot----------------")
    #新冠图像的密度图显示
    covid_ked()
    # 可视化数据集的切片
    ...
    # local_train_dataset(args, ensemble_train, ensemble_test)

   #本地训练
    print("-----------------Local Train ----------------")
    # local_train_dataset(args, train_dataset1, test_dataset1)

    print("-----------------Supervised Train NoIID----------------")
    # 监督学习下的联邦学习 model可以设置为联邦平均FedAvg，联邦近邻FedProx,联邦模型对比MOON,聚合归一化局部模型FedNova,FedBN
    mode = 'FedNova'
